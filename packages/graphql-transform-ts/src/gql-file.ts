 ...
 * 5. process
 * 6. ready for getJs, getTs, getDTs
 */
...
export class GqlFile extends Source {
  ...
  private packageRelativeName: string | null = null;
   ...
   */
  constructor(body: string, name: string, config: GraphqlTsConfig) {
    super(body, name, config && config.locationOffset);
    this.config = config;
    this.genFileName = `${name}.ts`;
    this.lines = body.split(/\r\n|\r|\n/g);
    console.log(`[graphql-ts] processing ${this.getPackageRelativeName()}`);
  ...
  getPackageRelativeName(): string {
    if (this.packageRelativeName) {
      return this.packageRelativeName;
    }

    // this.packageRelativeName = config.rootDir ? relative(config.rootDir, name) : name;
  ...
  private getTypeImport(module: string, identifier: string | null = null, alias?: string): string {
  ...
  getTsCode(): string {
    ...
    const packageRelativeName = this.getPackageRelativeName();
    ...
    const codeBlocks = [
      `// #sha384: ${hash}`,
      `// #body: ${packageRelativeName}`,
      getWarning(packageRelativeName),
      this.getImportTsSource(),
      ...includeImports,
      ...this.exports,
      `export const sha384Hash = ${JSON.stringify(hash)};`,
      shortNotice,
      `export const src = \`${this.body.replace(/[`$\\]/g, (m) => `\\${m}`)}\``,
      shortNotice,
      `export const source = /*@__PURE__*/ new ${idSource}(`,
      `  src,`,
      `  ${JSON.stringify(packageRelativeName)}`,
      `);`,
      shortNotice,
    ...
    if (this.config.prettify) {
      const prettierConfig = resolveConfig.sync(this.genFileName, {
        editorconfig: true,
        useCache: true,
      });
